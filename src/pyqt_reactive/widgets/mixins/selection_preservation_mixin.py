...
def preserve_selection_during_update(
    ...
    # Restore selection
    if current_id:
        restore_selection_by_id(list_widget, current_id)
...
def restore_selection_by_id(
    ...
    if current_item is not None:
        ...
        if current_data is not None and str(current_data) == item_id:
            return

    for index in range(list_widget.count()):
        item = list_widget.item(index)
        item_data = item.data(Qt.ItemDataRole.UserRole)
        if item_data is not None and str(item_data) == item_id:
